...
**Chunk size:** One chunk per member (their profile fields + their "Facts" list), plus one group-header chunk per document (debut, fandom, colors, SNS). Any member whose chunk would exceed **500 tokens** (a safety margin under bge-small's 512-token limit) is packed into sequential sub-chunks on line boundaries, each re-prefixed with a `<Group> — <Member>` context header. Final token distribution: **min 33, median 396, max 500.**

**Overlap:** None. Because splits happen on natural member/line boundaries and every (sub-)chunk repeats the `<Group> — <Member>` header, each chunk already carries the context needed to be retrievable on its own, so overlap wasn't necessary.

**Preprocessing before chunking:** Removed kprofiles boilerplate from every document — the site-header block (site name, author, reading time), image-caption lines (e.g. "… Concept Photo", "RM of BTS", "Jisoo-Blackpink-2025"), "Show more … fun facts" links, and empty "Official Logo:" placeholders. Substantive content (profile fields, facts, the `Note 2–5` sourcing blocks, SNS handles) was kept. Loading and blank-line normalization are in `ingest.py`; chunking is in `chunk.py`.
...
**Model used:** `BAAI/bge-small-en-v1.5` via `sentence-transformers`, running locally (384-dim, 512-token context, cosine similarity). Vectors are normalized and stored in a persistent **ChromaDB** collection configured with `hnsw:space = cosine`. Per BGE's guidance, the *query* (not the documents) is prefixed with `"Represent this sentence for searching relevant passages: "`. I chose bge-small over the common default `all-MiniLM-L6-v2` specifically because MiniLM truncates at **256 tokens** — half my chunks would lose their tail facts — whereas bge-small's 512-token window fits a full member chunk and matches or beats MiniLM on retrieval benchmarks while still running free and offline.

**Production tradeoff reflection:** If cost weren't a constraint, the biggest factor for this domain is **multilingual support**. The sources mix Korean, Japanese, and Chinese names and stage names (e.g. "Yu Ji-min" / "유지민" / "Karina"), and an English-only model can miss a query phrased with a different script or romanization. A multilingual model (e.g. `bge-m3` or `multilingual-e5-large`) or a high-accuracy API model (OpenAI `text-embedding-3-large`, Voyage) would improve cross-script recall and offer longer context — enough to embed an entire group as one chunk instead of per-member. The cost is added latency, a per-query network dependency, and sending data off-machine; bge-small is the right default here because it's fast, free, private, and accurate enough, even though it isn't the most capable option available.
...
```
Each retrieved chunk is wrapped as `[Document N — <Group> / <Member> (source: <file>)]` so the model can see provenance, and the user question is appended after the context block. Asked an out-of-scope question (e.g. "Who is the leader of TWICE?", a group not in the corpus), the system returns the exact refusal string rather than hallucinating.

**How source attribution is surfaced in the response:** Attribution is **programmatic, not model-dependent**. After generation, `ask()` collects the unique source filenames of the retrieved chunks (in retrieval order) and returns them as a `sources` list, which the Gradio UI shows in a separate **"Retrieved from"** panel. This is provenance of what was searched, so it never relies on the model remembering to cite. When the model returns the "not enough information" refusal, the source list is suppressed so we don't attribute a non-answer.
...
|---|----------|-----------------|------------------------------|-------------------|-------------------|
| 1 | Who is the maknae of aespa and what is her nationality? | NingNing; Chinese | "The maknae of aespa is NingNing, and her nationality is Chinese." | Partially relevant (correct chunk at rank 5; a BLACKPINK chunk intruded) | Accurate |
| 2 | What does aespa's name come from? | "æ" from "Avatar X Experience" + "aspect" | Quoted the full origin verbatim from the source | Relevant (rank 1, dist 0.234) | Accurate |
| 3 | When did 2NE1 disband and which member left before that? | Minzy left Apr 2016; disbanded Nov 25 2016 | "2NE1 disbanded on November 25, 2016. Before that, Minzy left the band in April 2016." | Relevant (rank 1) | Accurate |
| 4 | What individual agencies did the BLACKPINK members set up? | Jennie→ODD ATELIER, Lisa→LLOUD, Jisoo→BLISSOO, Rosé→THEBLACKLABEL | Listed all four correctly | Relevant (rank 1) | Accurate |
| 5 | Which girl groups are under YG Entertainment? | 2NE1 and BLACKPINK | "2NE1, BLACKPINK" | Partially relevant (correct groups rank 1–2; SM groups also retrieved) | Accurate |

**Retrieval quality:** Relevant / Partially relevant / Off-target
**Response accuracy:** Accurate / Partially accurate / Inaccurate

All five answers were accurate. Notably, Q5's *generation* stayed correct even though retrieval surfaced two non-YG groups — the grounding prompt led the model to exclude SNSD and S.E.S from the answer.
...
**Root cause (tied to a specific pipeline stage):** This is an **embedding/retrieval** limitation. Every group-header chunk is dominated by shared topic vocabulary ("South Korean girl group", "Entertainment", "debuted", "fandom"), and the one discriminating token — the agency name *YG* vs *SM* — is a tiny part of a ~300-token embedding. Dense cosine similarity captures overall topical closeness, not a precise attribute filter, so all "girl group + agency" headers cluster together and the agency distinction barely moves the distance. The same effect appears in Q1, where "maknae + nationality" pulled BLACKPINK's Lisa above aespa's NingNing.

**What I would change to fix it:** Add an `agency` field to chunk metadata (parsed from "under YG Entertainment" in the group description) and apply a ChromaDB `where={"agency": "YG Entertainment"}` filter when the query names an agency — turning a fuzzy semantic match into an exact metadata filter. A lighter-weight alternative is enriching the chunk header with the discriminating attribute (e.g. `aespa — Group Info (SM Entertainment girl group)`) so the agency carries more embedding weight, and/or raising top-k so aggregation queries see more candidates. In this case the grounded generation step already compensated and produced the right answer, but retrieval precision is the correct place to fix it.
...
- *What I gave the AI:* My Chunking Strategy section from `planning.md` plus a full sample document so it could see the real kprofiles structure (group header + repeating per-member blocks).
- *What it produced:* A `chunk_text()` that detected member boundaries using the `Stage Name:` field.
- *What I changed or overrode:* That approach missed irregular members with no `Stage Name:` line (2NE1's Minzy, three H.O.T. members) and falsely flagged the group-description paragraph as a member. I directed it to switch to a structural rule (a short, colon-free name line following a section break, confirmed by a nearby profile field) with a length guard, and — after we measured exact token counts — to add token-aware packing so no chunk exceeds bge-small's 512-token limit.
...
- *What I changed or overrode:* The measured data changed my decision from "keep whole members and accept truncation" to token-aware packing, and confirmed the switch to `bge-small-en-v1.5` (512-token window). I also overrode the Gradio CSS approach after it found that `Blocks(css=)` is silently dropped in Gradio 6, moving to `launch(css_paths=...)`.
